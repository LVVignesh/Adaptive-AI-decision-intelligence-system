...
def _compute_reward(effective_gains: Dict[str, float], actual_allocations: Dict[str, int], current_demands: Dict[str, int]) -> float:
    ...
    penalty = waste * 0.005
    # 🚨 Phase 2 High-Precision Compliance: Strictly within (0, 1).
    EPS = 1e-6
    return float(max(EPS, min(1.0 - EPS, base_reward - penalty)))
    ...
    def step(self, action: Any, timeout_s: Optional[float] = None, **kwargs) -> TaskObservation:
        ...
        if done:
            if all(d < 5 for d in demands.values()):
                # Apply victory bonus and clamp to ensure reward is strictly in (0, 1)
                reward = min(1.0 - 1e-6, reward + 0.20)
            ...
            final_score = round(max(1e-6, min(1.0 - 1e-6, state.total_score / 5)), 6)
